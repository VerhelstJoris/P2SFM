...
namespace Helper 
{
	...
	bool ReadSparseMatrixFromMat(const char* filePath, const char* varName, MatrixColSparse<double,int64_t>* sparseMat)
	{
		mxArray* matrixMat = ReadMxArrayFromMat(filePath, varName);
		...
		if (matrixMat != NULL && mxIsDouble(matrixMat) && !mxIsEmpty(matrixMat))
		{
			...
			Eigen::Map<MatrixColSparse<double,int64_t>> spMap(nrRows, nrCols, nnz, colIndex, rowIndex, pr,0);
