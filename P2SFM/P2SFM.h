...
namespace P2SFM 
{
	namespace EigenHelpers
	{
		...
		{				 
			Vector<int> point_path((pathway.block(0,first,1,last+1-first).array() >= 0).count());
			Vector<int> cam_path((pathway.block(0, first, 1, last+1-first).array()<0).count());
		...
		*/
			...
			const double distance_threshold)
		{
			...
			for (size_t i = 0; i <= last_iteration; i++)
			{
				...
				//std::vector<int> test_set{ 35, 7, 49, 52, 98, 61, 121, 27 };	//TEST SET
	...
	*/
		...
		const Eigen::Vector2i initial_views)
	{
		...
		//initial views have 1 added to them to ensure the value is always negative (0 is not)
		pathway << initial_points(0), -(initial_views(0)+1), initial_points(1), -(initial_views(1)+1), initial_points.block(0, 2, 1, initial_points.cols() - 2);//rest of initial_points
	...
	*/
		...
		for (size_t i = 0; i < pairs_sorted.size(); i++)
		{
				...
				&& (firstViewDense.rows() == secondViewDense.rows() || firstViewDense.cols() == secondViewDense.cols()))
			{
				...
				if (succesfulEstimation)
				{
					...
					init_points= EigenHelpers::MaskVector(init_points, (inliers.array() != (MatrixType)0));
		...
		const int num_points)
	{
		...
		std::tie(points, cams,std::ignore,std::ignore) = EigenHelpers::GetPointsCamsPathway(solved.pathway, 0,solved.pathway.size()-1);
		...
		if (solved.cameras.rows() != 3 * num_views)
		{
			...
			{
				newCameras.row(3 * cams(i))   = solved.cameras.row(i*3+ 0);
				newCameras.row(3 * cams(i)+1) = solved.cameras.row(i*3 + 1);
				newCameras.row(3 * cams(i)+2) = solved.cameras.row(i*3 + 2);
		...
		if (solved.points.cols() != num_points)
		{
			...
			{
				tempPoints.col(points(i)) = solved.points.col(i);
			}
		...
		{
			std::vector<Eigen::Triplet<MatrixType, IndexType>> tripletList;
			tripletList.reserve(solved.pathway.size() * cams.size());
			...
			for (size_t i = 0; i <points.size(); i++)
			{
				...
				{
					tripletList.push_back(Eigen::Triplet<MatrixType, IndexType>(cams(j), points(i), 1));
				}
			}
	...
	int TryAddingViews(
		...
		const int level,
		ViewsPointsProjections<MatrixType, IndexType>& projections,
		Vector<int>& rejected_views,
		MatrixColSparse<MatrixType, IndexType>& inliers,
		int& last_path,
		const Options& options = Options())
	{
		...
		for (size_t i = 0; i < eligibles.size(); i++)
		{
			...
			visible_points.conservativeResize(count);
			...
			if (options.robust_estimation)
			{
				...
				std::tie(estim,inlier_points) = EstimateRobust(data,pinv_meas, projections,visible_points,eligibles(i),rejected_views(eligibles(i)),normalisations,img_meas,level,true,options);
			}
			else
			{
				//care only about first of 4 outputs
				estim = std::get<0>(EstimateView(data, pinv_meas, projections.points, visible_points, eligibles(i), visible_points.size()));
			...
			else
			{
				...
				{
					projections.cameras.row( eligibles(i) * 3  +j) = estim.block(4*j, 0, 4, 1).transpose();
				...
				if (projections.fixed.rows() < inlier_points.size())
				{
					//output.fixed.makeCompressed();
					projections.fixed.conservativeResize(inlier_points.size(), projections.fixed.cols()); //keeps elements 
				}
				...
				for (size_t j = 0; j < inlier_points.size(); j++)
				{
					...
					projections.fixed.coeffRef(j, last_path) = inlier_points(j);
				}
			
				//add 1 to ensure negative
				projections.pathway(last_path) = -(eligibles(i)+1);
		...
		const Options& options = Options())
	{
		...
		{
			int current_id = ( -solveOutput.pathway(cameras_id(i)))-1;
			...
			//fixed_points.conservativeResize(count);
			...
			//std::cout << "visible " << visible_points.block(0, 0, 1, id) << "       "<< id<< std::endl;
	...
	Refinement(
		...
		const Options& options = Options())
	{
		Vector<int> sub_path = solveOutput.pathway.block(0, first_path, 1, last_path +1 - first_path);

		MatrixColSparse<int, IndexType> sub_fixed(solveOutput.fixed.rows(), last_path +1 - first_path);	//size declaration

		//std::cout << "sub_fixed" << sub_fixed.rows() << ", " << sub_fixed.cols() << std::endl;
		//std::cout << "fixed" << solveOutput.fixed.rows() << ", " << solveOutput.fixed.cols() << std::endl;
		...
		Vector<int> known_points;	//actual values stored
		Vector<int> points_id;	//id's
		Vector<int> camera_id;	//id's
 		Vector<int> known_cams;	//actual value

		//retrieve both tuples
		std::tie(known_points,known_cams, points_id, camera_id ) = EigenHelpers::GetPointsCamsPathway(solveOutput.pathway,first_path, last_path);
		...
		//remove columns/points not in known_points
		EigenHelpers::SparseRemoveCols(vis_copy, known_points);

		//assigned to a different matrix for aliasing reasons
		MatrixColSparse<MatrixType, IndexType> vis_transposed = vis_copy.transpose();	//remove rows with cam id

		//remove rows/views not in known_cams
		EigenHelpers::SparseRemoveCols(vis_transposed, known_cams);
		vis_copy = vis_transposed.transpose();	//transpose back to regular
		vis_copy.prune(MatrixType(0));	//seems cursed
		...
		for (; i < max_iterations; i++)
		{
			...
			if (start_cameras)
			{
				new_output.cameras = ReEstimateAllViews(data, pinv_meas, vis_copy, new_output, camera_id,options);
				new_output.points = ReEstimateAllPoints(data, pinv_meas, vis_copy, new_output, points_id,options);
			}
			else //reversed order
			{
				//std::cout << "REESTIMATE POINTS" << std::endl;
				new_output.points = ReEstimateAllPoints(data, pinv_meas, vis_copy, new_output, points_id, options);
				//std::cout << "camera id: " << camera_id << std::endl << camera_id.size() << std::endl;
				//std::cout << "REESTIMATE VIEWS" << std::endl;
				new_output.cameras = ReEstimateAllViews(data, pinv_meas, vis_copy, new_output, camera_id,options);
				//std::cout << "reestimated all views" << std::endl;
			...
			if (i+1 >= options.min_iter_refine && (change_points+change_points)/2 < min_change)
			{
				break;
			}
		}
	...
	*/
		...
		const Options& options = Options())
	{
		...
		int last_path = std::get<1>(expand_result) -1; //LAST PATH REPRESENTS THE LAST ID -> size() -1
		...
		Vector<int> cam_path;
		std::tie(point_path, cam_path, std::ignore, std::ignore) = EigenHelpers::GetPointsCamsPathway(new_projections.pathway,0,last_path);
		...
		//last path +1 as last_path is ID not amount
		int num_known_views  = (new_projections.pathway.block(0, 0, 1, last_path+1).array() < 0).count();
		int num_known_points = (new_projections.pathway.block(0, 0, 1, last_path+1).array() > 0).count();	//>0 to avoid empty values appended at the end
			...
			(level_changed==true || num_added_views + num_added_points > 0) )
		{
			...
			{
				std::tie(point_path, cam_path, std::ignore, std::ignore) = EigenHelpers::GetPointsCamsPathway(new_projections.pathway,0,last_path);	//can be skipped if this is the first loop
			}
			...
			if (num_known_views < num_views)
			{
				...
				if (eligibles.size() != 0)
				{
					...
					if (num_added_views > 0)
					{
						...
						//std::cout << "pathway block (init_refine, last_path +1 - init_refine): "<< new_projections.pathway.block(0, init_refine, 1, last_path + 1 - init_refine) << "     " << last_path + 1 - init_refine <<  std::endl;
			...
			if (eligible_points.size() != 0)
			{
				...
				//std::cout << "pathway: " << new_projections.pathway << std::endl;
				...
				if (num_added_points > 0)
				{
					...
					//std::cout << new_projections.pathway.block(0, init_refine, 1, last_path - init_refine) << std::endl;
		...
		new_projections.pathway.conservativeResize(last_path +1);
		MatrixColSparse<int, IndexType> sub_fixed(new_projections.fixed.rows(), last_path +1 );
	...
	* data : Structure containing all data in the following fields :
	...
	>
		...
		const MatrixDynamicDense<MatrixType>& sizeInput,
		const Options& options = Options())
	{
		...
		//PREPARE DATA
		//======================================================
		std::tie(hom_measurements, data, pinv_meas, norm_meas, normalisations, visibility) = P2SFM::PrepareData(measurements, options);

		auto t2 = Clock::now();
		if (options.debuginform >= P2SFM::Options::InformDebug::REGULAR)
		{
		...
		std::tie(affinity, view_pairs) = P2SFM::PairsAffinity(hom_measurements, visibility, (sizeInput).cast<int>(), options);
		...
		for (size_t i = 0; i < options.max_models; i++)
		{
			iteration++;
			...
			std::tie(projections_initial, success) = P2SFM::Initialisation(norm_meas, visibility, affinity, view_pairs, estimated_views, options);
			if (!success)
			{
				if (options.debuginform >= P2SFM::Options::InformDebug::REGULAR)
			...
			std::tie(projections_final, inliers) = P2SFM::Complete(data, pinv_meas, visibility, normalisations, hom_measurements, sizeInput, centerInput, projections_initial, options);
			auto t5 = Clock::now();
			...
			{
				//std::cout << "FINAL REFINEMENT 0-" << projections_final.pathway.size() -1 << std::endl;
				projections_final = P2SFM::Refinement(data, pinv_meas, inliers, projections_final, 0, projections_final.pathway.size() - 1, false, P2SFM::EigenHelpers::REFINEMENT_TYPE::FINAL, "Final", options);
				if (options.debuginform >= P2SFM::Options::InformDebug::REGULAR)
				{
			...
			if (estimated_views.size() == visibility.rows())
			{
				if (options.debuginform >= P2SFM::Options::InformDebug::REGULAR)
				{
					std::cout << "all views estimated" << std::endl;
				}
				break;
			}
			else if (options.debuginform >= P2SFM::Options::InformDebug::REGULAR)
			{
